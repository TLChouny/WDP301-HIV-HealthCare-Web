...
const RoleManagement: React.FC = () => {
  ...
  const filteredAndSortedUsers = [...users] // Tạo một bản sao để không làm thay đổi state 'users' trực tiếp
    .filter((user) => {
      ...
      const matchesStatus =
        selectedStatus === "all" || getStatus(user) === selectedStatus;
  ...
  const allRoles = ["admin", "doctor", "staff", "tester", "user"]; // Thêm 'tester' vào danh sách
  ...
  const handleEditUser = (user: User) => {
    ...
    setTimeout(() => {
      form.setFieldsValue({
        ...
        userDescription:
          user.role === "doctor" ? user.userDescription || "" : undefined,
        isVerified: user.role === "admin" ? true : user.isVerified, // Admin luôn có trạng thái true
      });
    }, 0);
  };
  ...
  const buildUpdatePayload = (values: any) => {
    const payload: any = {
      ...
      isVerified: values.role === "admin" ? true : values.isVerified, // Admin luôn có trạng thái true
    };
              ...
              form.getFieldValue("role") === "doctor" && (
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Vui lòng nhập mô tả cho bác sĩ",
                    },
                  ]}
                >
      ...
      >
        <Form
          ...
          onFinish={async (values) => {
            setAdding(true);
            try {
              // Mặc định role là 'user' khi thêm mới
              const userData = {
                ...values,
                role: "user",
              };
